...
def payments(request):
    ...
    for item in cart_items:
        ...
        orderproduct.save()
        # Get the cart item variations
        cart_item = CardItem.objects.get(id=item.id)
        ...
        product_obj.save()
    # Clear the cart
    CardItem.objects.filter(user=request.user).delete()
    # Clear the cart
    CardItem.objects.filter(user=request.user).delete()
...
def place_order(request, total=0, quantity=0):
    ...
    cart_items = CardItem.objects.filter(user=current_user)
